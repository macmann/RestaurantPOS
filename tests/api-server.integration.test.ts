...
async function runApiIntegration(): Promise<void> {
  ...
  const autoLinkedMenuItem = await adminCreateItem({ branchId, categoryId: category.id, name: 'API Auto Linked Rice', price: 9, prepStation: 'kitchen', inventoryCurrentStock: 1, isAvailable: true });
  ...
  try {
    ...
    assert(autoLinkedMenuItem.inventoryItemId, 'Menu item creation should return an auto-linked inventory item id.');
    const autoLinkedOrderResponse = await apiRequest<{ data: { id: string; version: number } }>(server.baseUrl, '/api/orders', {
      method: 'POST',
      token: waiter.token,
      body: { branchId, serviceMode: 'takeout', takeoutName: 'Auto Linked API Guest', items: [{ menuItemId: autoLinkedMenuItem.id, quantity: 1 }] },
    });
    assert(autoLinkedOrderResponse.status === 201, `Creating an auto-linked inventory order should return 201, got ${autoLinkedOrderResponse.status}.`);
    const autoLinkedStatusResponse = await apiRequest(server.baseUrl, `/api/orders/${autoLinkedOrderResponse.body.data.id}/status`, {
      method: 'POST',
      token: cashier.token,
      body: { expectedVersion: autoLinkedOrderResponse.body.data.version, nextStatus: 'in_preparation' },
    });
