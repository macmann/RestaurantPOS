...
export function buildManagerRouter(): Router {
  ...
  router.patch('/menu/items/:id/promotional', authorize(Actions.ManageMenu), route(async(req,res)=>{const storeId=managerStore(req);assertStore(req,await getItemById(text(req.params.id,'id')));const record=await withTransaction(async(client)=>queueMenuEvent(client,storeId,'MENU_ITEM_UPDATED',await AdminMenuApi.setPromotional(text(req.params.id,'id'),Boolean((req.body as any).isPromotional),{source:'CLOUD_MANAGER',actorId:req.user?.id})));res.json({data:record});}));
