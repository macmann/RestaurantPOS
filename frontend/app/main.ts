...
async function renderStaffSettings(isSuperadminPanel = false): Promise<HTMLElement> {
  ...
  if (isSuperadminPanel) {
    ...
    launchpad.innerHTML = `
      <button type="button" data-target="#/bill-settings"><span>Bill & printer setup</span><small>Receipt branding, tax info, and device routing</small></button>
      <button type="button" data-target="#/menu-admin"><span>Menu setup</span><small>Categories, pricing, promos, and availability</small></button>
      <button type="button" data-target="#/table-admin"><span>Table layout</span><small>Floor plan, sections, covers, and table status</small></button>
      <button type="button" data-target="#/reports"><span>Reports</span><small>Sales, financial summary, and inventory usage</small></button>
    `;
